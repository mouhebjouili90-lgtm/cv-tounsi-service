...
mindmap
  root((CV Tounsi))
    Différenciation
      IA intégrée
      3 formats (TN, Canada, Europass)
      Mode Étudiant / Professionnel
      5 langues
    Prix
    ...
    Marché
      Pas de concurrent IA local
      60% population < 35 ans
      Forte demande immigration Canada
      Saison PFE / stages
```
...
graph LR
    subgraph "Navigateur Client"
        A["Meta Pixel<br>(Browser)"] -->|PageView, ViewContent<br>InitiateCheckout| C["Meta Events Manager"]
        B["GA4 Tag<br>(gtag.js)"] -->|page_view, purchase| D["Google Analytics 4"]
    end

    subgraph "Serveur CV Tounsi"
        E["Conversions API<br>(Server-Side)"] -->|Purchase, Lead<br>avec event_id| C
    end

    subgraph "Meta"
        C -->|Deduplication<br>event_id match| F["Optimisation<br>Algorithme Ads"]
    end
...
```mermaid
graph TB
    subgraph "🔵 CAMPAGNE 1 — ACQUISITION (TOF)"
        direction TB
        A1["🎯 Objectif : Conversions<br>(Optimiser pour Purchase)"]
        A2["Budget : 50% du total<br>~10-15 TND/jour"]
        A3["Audience : Broad<br>20-35 ans, Tunisie"]
        
        subgraph "Ad Sets"
            B1["Ad Set 1<br>🎬 Vidéo Étudiants"]
            B2["Ad Set 2<br>🖼️ Image Comparatif"]
            B3["Ad Set 3<br>🇨🇦 Angle Canada"]
        end
    ...
    subgraph "🟡 CAMPAGNE 2 — RETARGETING (MOF)"
        direction TB
        C1["🎯 Objectif : Conversions"]
        C2["Budget : 30% du total<br>~5-10 TND/jour"]
        C3["Audience : Visiteurs 7j<br>qui n'ont pas acheté"]
        
        subgraph "Ad Sets MOF"
            D1["Ad Set 1<br>📊 Témoignages"]
            D2["Ad Set 2<br>⏰ Urgence/FOMO"]
        end
    ...
    subgraph "🔴 CAMPAGNE 3 — CONVERSION (BOF)"
        direction TB
        E1["🎯 Objectif : Conversions"]
        E2["Budget : 20% du total<br>~3-5 TND/jour"]
        E3["Audience : BuilderStarted<br>mais pas Purchase"]
        
        subgraph "Ad Sets BOF"
            F1["Ad Set 1<br>🎁 Offre limitée"]
        end
...
```mermaid
graph LR
    subgraph "Métriques Créatif"
        A["CTR<br>(Taux de clic)"]
        B["Hook Rate<br>(3s video views / impressions)"]
        C["CPM<br>(Coût / 1000 impressions)"]
    end
    
    subgraph "Métriques Funnel"
        D["CPC<br>(Coût par clic)"]
        E["LP Conv. Rate<br>(Visiteurs → Builder)"]
        F["Builder → Purchase<br>(Taux de conversion)"]
    end
    
    subgraph "Métriques Business"
        G["CPA<br>(Coût par vente)"]
        H["ROAS<br>(Revenu / Dépense)"]
        I["Profit Net<br>(Revenu - Ads)"]
    end
